-- Align production databases that already applied the original 20260530023000 migration
-- with the current production auth/admin schema.

ALTER TABLE public.profiles
  ADD COLUMN IF NOT EXISTS "Deleted" integer NOT NULL DEFAULT 0 CHECK ("Deleted" IN (0, 1)),
  ADD COLUMN IF NOT EXISTS "createdDT" timestamptz NOT NULL DEFAULT now(),
  ADD COLUMN IF NOT EXISTS "updatedDT" timestamptz NOT NULL DEFAULT now();

ALTER TABLE public.auth_provider_options
  ADD COLUMN IF NOT EXISTS "Deleted" integer NOT NULL DEFAULT 0 CHECK ("Deleted" IN (0, 1)),
  ADD COLUMN IF NOT EXISTS "createdDT" timestamptz NOT NULL DEFAULT now(),
  ADD COLUMN IF NOT EXISTS "updatedDT" timestamptz NOT NULL DEFAULT now();
...
ALTER TABLE public.reservations
  ...
  ADD COLUMN IF NOT EXISTS paid integer NOT NULL DEFAULT 0 CHECK (paid IN (0, 1)),
...
ALTER TABLE public.fixed_reservations
  ADD COLUMN IF NOT EXISTS "Deleted" integer NOT NULL DEFAULT 0 CHECK ("Deleted" IN (0, 1)),
  ADD COLUMN IF NOT EXISTS "createdDT" timestamptz NOT NULL DEFAULT now(),
  ADD COLUMN IF NOT EXISTS "updatedDT" timestamptz NOT NULL DEFAULT now();
...
CREATE INDEX IF NOT EXISTS reservations_subject_id_idx ON public.reservations(subject_id) WHERE "Deleted" = 0;
CREATE INDEX IF NOT EXISTS admin_subjects_type_idx ON public.admin_subjects(subject_type) WHERE "Deleted" = 0;
CREATE INDEX IF NOT EXISTS admin_bulk_operations_status_idx ON public.admin_bulk_operations(status) WHERE "Deleted" = 0;
CREATE INDEX IF NOT EXISTS admin_bulk_operation_items_operation_idx ON public.admin_bulk_operation_items(bulk_operation_id) WHERE "Deleted" = 0;

DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'reservations_bulk_operation_id_fkey'
  ) THEN
    ALTER TABLE public.reservations
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'reservations_subject_id_fkey'
  ) THEN
    ALTER TABLE public.reservations
...
DROP TRIGGER IF EXISTS profiles_set_updated_dt ON public.profiles;
DROP TRIGGER IF EXISTS auth_provider_options_set_updated_dt ON public.auth_provider_options;
DROP TRIGGER IF EXISTS facility_config_set_updated_dt ON public.facility_config;
DROP TRIGGER IF EXISTS operating_hours_set_updated_dt ON public.operating_hours;
DROP TRIGGER IF EXISTS resources_set_updated_dt ON public.resources;
DROP TRIGGER IF EXISTS reservations_set_updated_dt ON public.reservations;
DROP TRIGGER IF EXISTS closures_set_updated_dt ON public.closures;
DROP TRIGGER IF EXISTS fixed_reservations_set_updated_dt ON public.fixed_reservations;
DROP TRIGGER IF EXISTS admin_subjects_set_updated_dt ON public.admin_subjects;
DROP TRIGGER IF EXISTS admin_bulk_operations_set_updated_dt ON public.admin_bulk_operations;
DROP TRIGGER IF EXISTS admin_bulk_operation_items_set_updated_dt ON public.admin_bulk_operation_items;

DROP POLICY IF EXISTS "Public can read non-sensitive facility config" ON public.facility_config;
...
DROP POLICY IF EXISTS "Public can read enabled auth providers" ON public.auth_provider_options;
DROP POLICY IF EXISTS "Users can read own profile" ON public.profiles;
DROP POLICY IF EXISTS "Users can read own reservations" ON public.reservations;
DROP POLICY IF EXISTS "Approved users can create own reservations" ON public.reservations;
DROP POLICY IF EXISTS "Admins manage application data" ON public.profiles;
DROP POLICY IF EXISTS "Admins manage facility config" ON public.facility_config;
DROP POLICY IF EXISTS "Admins manage operating hours" ON public.operating_hours;
...
DROP POLICY IF EXISTS "Admins manage closures" ON public.closures;
DROP POLICY IF EXISTS "Admins manage fixed reservations" ON public.fixed_reservations;
DROP POLICY IF EXISTS "Admins manage auth provider options" ON public.auth_provider_options;
DROP POLICY IF EXISTS "Admins read active profiles" ON public.profiles;
DROP POLICY IF EXISTS "Admins insert profiles" ON public.profiles;
DROP POLICY IF EXISTS "Admins update profiles" ON public.profiles;
DROP POLICY IF EXISTS "Admins read active resources" ON public.resources;
...
DROP POLICY IF EXISTS "Admins read active reservations" ON public.reservations;
...
DROP POLICY IF EXISTS "Admins read active closures" ON public.closures;
DROP POLICY IF EXISTS "Admins insert closures" ON public.closures;
DROP POLICY IF EXISTS "Admins update closures" ON public.closures;
DROP POLICY IF EXISTS "Admins read active fixed reservations" ON public.fixed_reservations;
DROP POLICY IF EXISTS "Admins insert fixed reservations" ON public.fixed_reservations;
DROP POLICY IF EXISTS "Admins update fixed reservations" ON public.fixed_reservations;
DROP POLICY IF EXISTS "Admins read active auth provider options" ON public.auth_provider_options;
DROP POLICY IF EXISTS "Admins insert auth provider options" ON public.auth_provider_options;
DROP POLICY IF EXISTS "Admins update auth provider options" ON public.auth_provider_options;
DROP POLICY IF EXISTS "Admins read active temporary subjects" ON public.admin_subjects;
DROP POLICY IF EXISTS "Admins insert temporary subjects" ON public.admin_subjects;
DROP POLICY IF EXISTS "Admins update temporary subjects" ON public.admin_subjects;
DROP POLICY IF EXISTS "Admins read active bulk operations" ON public.admin_bulk_operations;
DROP POLICY IF EXISTS "Admins insert bulk operations" ON public.admin_bulk_operations;
DROP POLICY IF EXISTS "Admins update bulk operations" ON public.admin_bulk_operations;
DROP POLICY IF EXISTS "Admins read active bulk operation items" ON public.admin_bulk_operation_items;
DROP POLICY IF EXISTS "Admins insert bulk operation items" ON public.admin_bulk_operation_items;
DROP POLICY IF EXISTS "Admins update bulk operation items" ON public.admin_bulk_operation_items;
