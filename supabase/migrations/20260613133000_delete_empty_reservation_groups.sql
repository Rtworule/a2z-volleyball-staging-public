...
BEGIN
  ...
  FOR deleted_reservation IN
    ...
    RETURNING target.*
  LOOP
    IF deleted_reservation.reservation_group_id IS NOT NULL
      AND NOT deleted_reservation.reservation_group_id = ANY(affected_group_ids) THEN
      affected_group_ids := array_append(affected_group_ids, deleted_reservation.reservation_group_id);
    END IF;
    ...
    INSERT INTO public.admin_bulk_operation_items (
      ...
      CASE WHEN deleted_reservation.payment_status = 'paid' THEN 1 ELSE 0 END,
      'applied'
    );
