...
const demoViewers = [approvedMember, pendingMember, guestUser];
...
elements.sessionToggle.addEventListener("click", () => {
  const currentIndex = demoViewers.findIndex((viewer) => viewer.id === state.viewer.id);
  state.viewer = demoViewers[(currentIndex + 1) % demoViewers.length];
  state.selectedSlot = null;
  state.status = canViewerReserve(state.viewer)
    ? "Approved account restored. Choose a slot to reserve."
    : "This account cannot request reservations until it is approved.";
  render();
});
...
function render() {
  ...
  elements.accountStatus.textContent = accountStatusLabel(state.viewer);
  elements.accountStatus.classList.toggle("is-guest", !state.viewer.authenticated);
  elements.accountStatus.classList.toggle("is-blocked", state.viewer.authenticated && !canViewerReserve(state.viewer));
  elements.sessionToggle.textContent = "Switch account";
...
function renderRequest() {
  ...
  elements.bookingStatus.classList.toggle("is-warning", !canViewerReserve(state.viewer));
  ...
  elements.reserveButton.disabled = !availability.available || !canViewerReserve(state.viewer);
}
