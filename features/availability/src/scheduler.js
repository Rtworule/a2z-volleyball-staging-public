...
export class SchedulingService {
  constructor(options = {}) {
    ...
    this.bookings = options.bookings ? [...options.bookings] : [];
  ...
  createCourtBooking(request, actor, start, end) {
    ...
    return this.persistBooking({
      userId: request.userId ?? actor.id,
  ...
  assertWithinOperatingHours(start, end) {
    ...
    if (startParts.dateKey !== endParts.dateKey) {
      throw new BookingValidationError("Bookings must start and end on the same local day");
    }

    const hours = this.operatingHours[startParts.dayOfWeek];
    if (!hours || hours.closed) {
      throw new BookingConflictError("The center is closed on the requested day");
    }
    ...
    if (startMinutes < openMinutes || endMinutes > closeMinutes) {
      throw new BookingConflictError("Requested time is outside operating hours", {
        open: hours.open,
        close: hours.close
      });
    }
...
function dateAtLocalTime(anchorDate, time, timezone) {
  ...
  const utcGuess = new Date(`${parts.dateKey}T${String(hour).padStart(2, "0")}:${String(minute).padStart(2, "0")}:00.000Z`);
  const guessParts = zonedParts(utcGuess, timezone);
  const offsetMinutes = (Number(guessParts.hour) - hour) * 60 + (Number(guessParts.minute) - minute);
